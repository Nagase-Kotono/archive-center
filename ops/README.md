...
Status: Archive Center 3.5 packaging, preflight and recovery workspace.
...
- `install-windows.ps1 -InstallMariaDBRuntime -InstallDir <path>`: downloads the pinned official MariaDB 11.4.10 Windows ZIP, verifies its SHA-256, and installs it outside the Archive Center package. No administrator rights or system service registration is required.
...
- `build-full-package.ps1 -PackageVersion 3.5.0 -PackageKind managed -Zip -ForceRefresh`: creates the Windows auto-install package under `_dist`. It builds the Go backend and copies the host adapter, prompts, migrations, notices, and launch scripts, but does not bundle MariaDB, ChromaDB, DB, `.env.full.local`, logs, caches, or user data. First launch downloads the verified official MariaDB runtime into the per-user runtime directory. The historical script name is retained so updater and operator paths do not fork.
- `build-posix-managed-packages.ps1 -PackageVersion 3.5.0 -Zip -ForceRefresh`: cross-builds the Linux x64/arm64, macOS Intel/Apple Silicon and Android Termux arm64 managed packages.
