...
if ([string]::IsNullOrWhiteSpace($SessionId)) {
    $SessionId = "live-test-" + [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
}

Write-Host "Archive Center 2.0 live smoke"
...
$config = Invoke-Json POST "/config/update" $runtimeConfig

$complete = Invoke-Json POST "/complete-turn" @{
    chat_session_id = $SessionId
    turn_index = 1
    user_input = "Live test user asks the archive to remember a brass key."
    assistant_content = "Live test assistant records that the brass key is under the blue lantern."
    context_messages = @(
        @{ role = "user"; content = "Remember the brass key." },
        @{ role = "assistant"; content = "The brass key is under the blue lantern." }
    )
    client_meta = @{
        source = "live_test_pack"
    }
}
...
$rollback = Invoke-Json DELETE "/rollback/1?chat_session_id=$SessionId&req_source=live_test_pack"
...
$outFile = Join-Path $outDir ("live-smoke-" + $SessionId + ".json")
$report | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $outFile -Encoding UTF8

Write-Host ""
Write-Host "Live smoke report:"
Write-Host "  $outFile"
