...
func memoryDerivationSchemaStatements() []string {
	return []string{
  ...
  COMMENT='Host-observed accepted raw user/assistant source revisions; no inferred branch identity.'`,
		`ALTER TABLE memory_source_revisions ADD COLUMN IF NOT EXISTS active_logical_turn_slot VARCHAR(160) GENERATED ALWAYS AS (CASE WHEN lifecycle_state = 'active' THEN logical_turn_id ELSE NULL END) PERSISTENT`,
		`ALTER TABLE memory_source_revisions ADD UNIQUE INDEX IF NOT EXISTS uq_memory_source_active_turn (chat_session_id, active_logical_turn_slot)`,
		`ALTER TABLE memory_source_revisions ADD COLUMN IF NOT EXISTS derived_admission_state VARCHAR(30) NOT NULL DEFAULT 'pending'`,
		`ALTER TABLE memory_source_revisions ADD COLUMN IF NOT EXISTS derived_admission_version VARCHAR(120) NOT NULL DEFAULT ''`,
		`ALTER TABLE memory_source_revisions ADD COLUMN IF NOT EXISTS derived_extractor_version VARCHAR(120) NOT NULL DEFAULT ''`,
		`ALTER TABLE memory_source_revisions ADD COLUMN IF NOT EXISTS derived_index_version VARCHAR(120) NOT NULL DEFAULT ''`,
